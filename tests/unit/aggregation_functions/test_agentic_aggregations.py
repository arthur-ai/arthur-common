...
from arthur_common.aggregations.functions.agentic_aggregations import (
    ...
    AgenticInferenceCountAggregation,
...
def test_relevance_pass_fail_count_correct_values(
    ...
    for group in metric.numeric_series:
        for point in group.values:
            if point.value > 0:  # Only process non-zero values
                # Extract dimensions
                agent_name = next((dim.value for dim in group.dimensions if dim.name == "agent_name"), "unknown")
                metric_type = next((dim.value for dim in group.dimensions if dim.name == "metric_type"), "unknown")
                score_type = next((dim.value for dim in group.dimensions if dim.name == "score_type"), "unknown")
                result = next((dim.value for dim in group.dimensions if dim.name == "result"), "unknown")
    ...
    expected_values = {
        ...
        ("unknown", "QueryRelevance", "reranker_relevance_score", "pass"): 2,  # traces 1, 5
        ...
        ("unknown", "ResponseRelevance", "average", "pass"): 2,  # traces 1, 5 (no agent)
        ("agent_1", "ResponseRelevance", "average", "pass"): 1,  # trace 2
        ("agent_2", "ResponseRelevance", "average", "pass"): 1,  # trace 3
        ("unknown", "ResponseRelevance", "llm_relevance_score", "pass"): 2,  # traces 1, 5
        ("agent_1", "ResponseRelevance", "llm_relevance_score", "pass"): 1,  # trace 2
        ("agent_2", "ResponseRelevance", "llm_relevance_score", "pass"): 1,  # trace 3
        ("unknown", "ResponseRelevance", "reranker_relevance_score", "pass"): 2,  # traces 1, 5
        ("agent_1", "ResponseRelevance", "reranker_relevance_score", "pass"): 1,  # trace 2
        ("agent_2", "ResponseRelevance", "reranker_relevance_score", "pass"): 1,  # trace 3
        ...
        ("agent_3", "ResponseRelevance", "reranker_relevance_score", "fail"): 1,  # trace 4
...
def test_tool_pass_fail_count_correct_values(
    ...
    for group in metric.numeric_series:
        for point in group.values:
            if point.value > 0:  # Only process non-zero values
                # Extract dimensions
                agent_name = next((dim.value for dim in group.dimensions if dim.name == "agent_name"), "unknown")
                tool_metric = next((dim.value for dim in group.dimensions if dim.name == "tool_metric"), "unknown")
                result = next((dim.value for dim in group.dimensions if dim.name == "result"), "unknown")
    ...
    no_tool_count = sum(value for key, value in metric_data.items() if key[2] == "no_tool")
...
def test_inference_count_with_traces(
    get_agentic_dataset_conn: tuple[DuckDBPyConnection, DatasetReference]
):
    ...
    """
    conn, dataset_ref = get_agentic_dataset_conn
    aggregation = AgenticInferenceCountAggregation()
    metrics = aggregation.aggregate(conn, dataset_ref)

    # Should return exactly one metric
    assert len(metrics) == 1
    metric = metrics[0]
    assert metric.name == "inference_count"
...
def test_inference_count_no_traces(
    get_agentic_dataset_conn_no_metrics: tuple[DuckDBPyConnection, DatasetReference]
):
...
def test_inference_count_total(
    get_agentic_dataset_conn: tuple[DuckDBPyConnection, DatasetReference]
):
    ...
    expected_buckets = 5
    assert len(metric_data) == expected_buckets, f"Expected {expected_buckets} time buckets, got {len(metric_data)}"
...
def test_inference_count_time_buckets(
    get_agentic_dataset_conn: tuple[DuckDBPyConnection, DatasetReference]
):
    """Test that inference count correctly groups traces by time buckets.
...
def test_tool_selection_and_usage_by_agent_correct_values(
    ...
    for group in metric.numeric_series:
        for point in group.values:
            if point.value > 0:  # Only process non-zero values
                # Extract dimensions
                agent_name = next((dim.value for dim in group.dimensions if dim.name == "agent_name"), "unknown")
                selection_category = next((dim.value for dim in group.dimensions if dim.name == "selection_category"), "unknown")
                usage_category = next((dim.value for dim in group.dimensions if dim.name == "usage_category"), "unknown")
    ...
    for combo in expected_combinations:
        ...
        assert category_counts[combo] == 1, f"Expected count 1 for {combo}, got {category_counts[combo]}"
...
def test_agentic_metrics_over_time_correct_values(
    ...
    for metric in metrics:
        ...
        for series in metric.sketch_series:
            assert len(series.values) > 0, f"Sketch series for {metric.name} should have data points"
...
def test_relevance_pass_fail_count_time_buckets(
    ...
    for bucket, metrics in bucket_data.items():
        ...
        assert total_scores == 8, f"Expected count 8 for bucket {bucket}, got {total_scores}"
        
        # Verify that each bucket has the expected score types
        expected_score_types = {
            "average", "llm_relevance_score", "reranker_relevance_score", "bert_f_score"
        }
        actual_score_types = {key[2] for key in metrics.keys()}
        assert actual_score_types == expected_score_types, f"Bucket {bucket}: Expected score types {expected_score_types}, got {actual_score_types}"
        ...
        assert actual_metric_types == expected_metric_types, f"Bucket {bucket}: Expected metric types {expected_metric_types}, got {actual_metric_types}"
...
def test_tool_pass_fail_count_time_buckets(
    ...
    for bucket, metrics in bucket_data.items():
        total_scores = sum(metrics.values())
        assert total_scores == 2, f"Expected count 2 for bucket {bucket}, got {total_scores}"
        ...
        assert actual_tool_metrics == expected_tool_metrics, f"Bucket {bucket}: Expected tool metrics {expected_tool_metrics}, got {actual_tool_metrics}"
        ...
        assert actual_results.issubset(valid_results), f"Bucket {bucket}: Invalid result types {actual_results - valid_results}"
...
def test_tool_selection_and_usage_by_agent_time_buckets(
    ...
    for bucket, metrics in bucket_data.items():
        ...
        assert total_combinations == 1, f"Expected count 1 for bucket {bucket}, got {total_combinations}"
        
        # Verify that each bucket has valid selection and usage categories
        valid_selection_categories = {"correct_selection", "incorrect_selection", "no_selection"}
        ...
        assert actual_selection_categories.issubset(valid_selection_categories), f"Bucket {bucket}: Invalid selection categories {actual_selection_categories - valid_selection_categories}"
        assert actual_usage_categories.issubset(valid_usage_categories), f"Bucket {bucket}: Invalid usage categories {actual_usage_categories - valid_usage_categories}"
